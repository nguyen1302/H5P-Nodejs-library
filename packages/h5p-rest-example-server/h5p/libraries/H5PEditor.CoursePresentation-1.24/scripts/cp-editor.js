...
H5PEditor.CoursePresentation.prototype.generateForm = function (elementParams, type) {
  ...
  if (type === 'goToSlide') {
    // Hide all others
    self.showFields(elementFields, ['title', 'goToSlide', 'goToSlideType']); // TUNG , 'invisible'
  }
  else {
    ...
    hideFields.push('overallFeedback');
    hideFields.push('behaviour');
  ...
  // TUNG
  element.$form.find('.field-name-a11y').remove();
  setTimeout(function(){
    element.$form.find('.field-name-a11y').remove();
  }, 1000);
  return element;
