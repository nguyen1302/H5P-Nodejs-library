...
ns.Html = function (parent, field, params, setValue) {
    ...
    this.tags = ns.$.merge(
        ['br', 'wirisplugins'],
        this.field.tags || this.defaultTags
    );
};
ns.Html.first = true;

ns.Html.prototype.defaultTags = [
    'strong',
    'em',
    'del',
    'h2',
...
ns.Html.prototype.getCKEditorConfig = function () {
    ...
    // TAN FIX: Add MathType and ChemType plugins
    //   if (this.inTags("wirisplugins")) {
    // if (window.WirisPlugin) {
    config.plugins.push('MathType');
    //   config.toolbar.push("|", "mathType", "chemType");
    config.toolbar.push('|', 'MathType', 'ChemType');
    // } else {
    //   console.warn("Wiris plugin is not loaded!");
    // }
    //   }
    ...
    for (let index = 1; index < 7; index++) {
        if (this.inTags('h' + index)) {
            formats.push({
                ...
                view: 'h' + index,
                title: 'Heading ' + index,
                class: 'ck-heading_heading' + index
            });
        }
    }
    ...
    if (this.field.font !== undefined) {
        ...
        if (this.field.font.family) {
            ...
            if (this.field.font.family instanceof Array) {
                fontFamilies = [
                    'default',
                    ...this.field.font.family.map(
                        (font) => font.label + ', ' + font.css
                    )
                ];
            }
        ...
        if (this.field.font.background) {
            ...
            if (this.field.font.background instanceof Array) {
                config.fontBackgroundColor = {
                    colors: getColors(this.field.font.color)
                };
            }
        }
...
ns.Html.prototype.appendTo = function ($wrapper) {
    ...
    this.$input.focus(function () {
        ...
        // Remove placeholder
        that.$placeholder = that.$item
            .find('.h5peditor-ckeditor-placeholder')
            .detach();
        ...
        ClassicEditor.create(this, that.ckEditorConfig)
            .then((editor) => {
                ...
                const dropdownPanel =
                    editor.ui.view.toolbar._behavior.groupedItemsDropdown;
                ...
                if (initialData.includes('table-overflow-protection')) {
                    initialData = initialData.replace(
                        /<div class=\"table-overflow-protection\">.*<\/div>/,
                        ''
                    );
                ...
                editor.once('destroy', function () {
                    ...
                    if (
                        that.$placeholder.length !== 0 &&
                        (value === undefined || value.length === 0) &&
                        (that.value === undefined || that.value.length === 0)
                    ) {
                        that.$placeholder.appendTo(
                            that.$item.find('.ckeditor')
                        );
                    }
                    ...
                    if (
                        ...
                        !value.includes('table-overflow-protection')
                    ) {
                        that.value =
                            value +
                            '<div class="table-overflow-protection"></div>';
...
ns.Html.prototype.createHtml = function () {
    ...
    if (this.value !== undefined) {
        ...
        input +=
            '<span class="h5peditor-ckeditor-placeholder">' +
            this.field.placeholder +
            '</span>';
    }
    // Add overflow protection if table
    if (
        this.field.tags.includes('table') &&
        !input.includes('table-overflow-protection')
    ) {
...
ns.Html.prototype.validate = function () {
    ...
    let value =
        this.ckeditor !== undefined && this.ckeditor.getData() !== undefined
    ...
    if (!this.field.optional && !textValue.length) {
        ...
        if (!(this.inTags('img') && $value.find('img').length > 0)) {
            this.$errors.append(
                ns.createError(
                    ns.t('core', 'requiredProperty', {
                        ':property': ns.t('core', 'textField')
                    })
                )
            );
        }
